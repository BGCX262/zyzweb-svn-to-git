...
<div class="body-box">
<div class="rhead">
		<div class="rpos">当前位置: <c:if test="${t=='3'}">按党组织统计-党员</c:if><c:if test="${t=='2'}">按党组织统计-申请党员</c:if><c:if test="${t=='1'}">按党组织统计-党员与申请党员</c:if> - 列表</div>

	<div class="clear"></div>
</div>
<form id="jvForm" action="<c:if test="${t=='3'}">queryDzzDy.do</c:if><c:if test="${t=='2'}">queryDzzDyNot.do</c:if><c:if test="${t=='1'}">queryDzzDyAll.do</c:if>" method="post" style="padding-top:5px;">
...
<tr>
 ...
 <td><c:out value="${row.cservicetimes}"/></td>
  ...
  <td> <a href="/zyzinfo/queryZyzDyAll.do?order=desc&sort=cscore&zzid=<c:out value="${row.d0100}"/>">统计该党组织下志愿者 </a></td>
