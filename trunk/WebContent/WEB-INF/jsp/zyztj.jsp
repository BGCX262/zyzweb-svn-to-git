...
function checkSort(){
	if(document.getElementById('order').value==''){
		...
		document.getElementById('sort').value='cscore';
	}
}
</script>
</head>
...
<div class="body-box">
<div class="rhead">
	  <div class="rpos">当前位置: <c:if test="${t=='3'}">按党员统计</c:if><c:if test="${t=='2'}">按申请党员统计</c:if><c:if test="${t=='1'}">按党员与申请党员统计</c:if> - 列表</div> 

	<div class="clear"></div>
</div>
<form id="jvForm" action="<c:if test="${t=='3'}">queryZyzDy.do</c:if><c:if test="${t=='2'}">queryZyzDyNot.do</c:if><c:if test="${t=='1'}">queryZyzDyAll.do</c:if>" method="post" onsubmit="checkSort()" style="padding-top:5px;">
    <input type="hidden" name="order" id="order" value="" />
    <input type="hidden" name="sort" id="sort" value="" />
    志愿者姓名:              <input type="text" name="username" value="" style="width:100px"/>
...
<tr>
 ...
 <td><c:out value="${row.cservicetimes}"/></td>
  ...
  <td><a href="zyzinfoEdit.do?zyzid=<c:out value="${row.zyzid}"/>&actiontype=view">详情</a></td>
...
function _gotoPage(pageNo) {
	try{
		...
		tableForm.action="<c:if test="${t=='3'}">queryZyzDy.do</c:if><c:if test="${t=='2'}">queryZyzDyNot.do</c:if><c:if test="${t=='1'}">queryZyzDyAll.do</c:if>";
