...
<tr>
 ...
 <td><c:out value="${row.bz}"/></td>
	<td align="center">		
   <a href="chartmgrEdit.do?actiontype=view&id=<c:out value="${row.id}"/>" target="_blank" class="pn-opt">详情</a>  	
    <c:if test="${row.status =='2'}">
   |<a href="chartmgrEdit.do?actiontype=edit&id=<c:out value="${row.id}"/>" class="pn-opt">修改</a>
     </c:if>
     <c:if test="${row.status!='1'}">
   |<a href="chartmgrRemove.do?isfront=0&ids=<c:out value="${row.id}"/>" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a>
     </c:if>
   </td>
</tr>
</c:forEach>  
...
<input type="hidden" id="actiontype" name="actiontype" /> 
		<input type="submit" value="添加" onclick="this.form.action='chartmgrAdd.do';document.getElementById('actiontype').value='add'"/>
</form></div>
</form>
</div>
</body>
