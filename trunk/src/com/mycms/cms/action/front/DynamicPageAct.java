...
import static com.mycms.cms.Constants.TPLDIR_INDEX;
import static com.mycms.common.web.Constants.INDEX;
...
import com.mycms.cms.manager.assist.CmsSensitivityMng;
...
import com.mycms.common.page.Paginable;
import com.mycms.common.page.SimplePage;
import com.mycms.core.web.front.URLHelper;
import com.mycms.core.web.front.URLHelper.PageInfo;
...
public class DynamicPageAct {
	 ...
	 * 动态页入口
	 */
	@RequestMapping(value = "/**/*.jhtml", method = RequestMethod.GET)
			...
			if (paths[1].equals(INDEX)) {
				...
				try {
					...
					try {
						...
						log.debug("Channel path not found: {}", paths[1]);
						return FrontUtils.pageNotFound(request, response, model);
					}
					return content(paths[0],id, pageNo, params, info, request, response,
							model);
	...
	public String content(String channelPath,Integer id, int pageNo, String[] params,
			PageInfo info, HttpServletRequest request,
			HttpServletResponse response, ModelMap model) {
		Content content = contentMng.findById(id);content.getChannel().getPath();
		if (content == null) {
			log.debug("Content id not found: {}", id);
			return FrontUtils.pageNotFound(request, response, model);
		}else if(!content.getChannel().getPath().equals(channelPath)){
			log.debug("Content channel paht not found: {}", channelPath);
			return FrontUtils.pageNotFound(request, response, model);
		}
		...
		txt=cmsSensitivityMng.replaceSensitivity(txt);
